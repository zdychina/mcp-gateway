...
java -jar target/mcp-gateway-1.2.0.jar
```
...
顶部导航有「总览」和「网关」两个入口，未登录时整块不渲染。

### 4.1 总览页 `/ui/dashboard`

回答"最近整体怎么样"。它不回答"某一次调用怎么了"—— 那是调用记录页的事，
所以这一页一条正文、一个参数都不显示。

顶上一行是口径：时间范围（1 小时 / 24 小时 / 7 天 / 30 天）和网关筛选。
**改的是整页的口径，不是某一张图的** —— 所有图表来自同一个接口的同一份数据，
一次请求、一个加载状态，不会出现"总量对得上、分组对不上"的中间态。

四个数字回答**要不要紧**：调用量、成功率、失败（错误 + 超时）、P95 耗时。
没有样本时比率和分位显示为 `—` 而不是 0 —— 把"没有数据"和"真的是 0"画成一样会误导。

四块内容回答**问题在哪**：

- **调用量趋势** —— 按时间桶堆叠（48 小时以内按小时，更长按天），没有调用的桶补零。
  右上角「看表格」可以切成表格看具体数字，图表对读屏软件是不可读的，数字得有另一条路拿到
- **各网关调用量** —— 点柱子直接钻进那个网关（等同于在上面的下拉里选它）
- **子 MCP 调用量** —— 要先选一个网关；子 MCP 属于具体网关，跨网关放一起比没有意义
- **调用最多的工具** / **错误码分布** —— 前者是表格，后者是条形图

页底「全部网关」是跨网关的导航，**不受上面的网关筛选影响**，每行可以直接跳到它的调用记录。

> 图表配色跑过色觉障碍校验，不是照搬状态徽章的颜色。暗色下"超时/错误"那一对落在
> 需要第二重编码的区间，所以状态图一律带图例、堆叠段之间留缝，并且可以切成表格。

### 4.2 列表页 `/ui/gateways`

显示每个网关的名称、slug、**MCP 地址**、状态、子 MCP 数量、工具数量、更新时间；
可以新建和删除网关。卡片头上有搜索框，按名称、slug 或描述过滤。

MCP 地址那一列带复制按钮 —— 配 Agent 时要的就是这个地址，不必为了复制它点进详情页。
地址长会截断显示，全文在悬停提示和复制按钮里。

每行右侧除「详情」外还有「调用记录」，排障时不用先进详情再点一次。
表格下面一行汇总：合计多少个子 MCP、多少个聚合工具。
...
三个入口：详情页右上角「查看调用记录」、网关列表页的行内按钮、总览页「全部网关」表的行内按钮。
...
页头「列」按钮打开列菜单：勾选显示哪些内置列（开始时间、聚合工具、子 MCP、原工具名、
状态、耗时、错误码、错误摘要、结束时间、`trace_id`、`call_id`），用 ↑ ↓ 调顺序。

菜单下半部分可以**加一列正文字段**：选入参或返回，填一个 JSON Pointer（如 `/q`、
`/content/0/text`，留空表示整个文档），给它起个列名。服务端只回抽到的值，
**正文本身仍然不会进列表**。最多 4 列，每格最长 200 字符。抽不到值时分三种情况显示
（路径不对 / 正文过大 / 不是 JSON），不会一律画成「—」—— 那会让人以为是同一回事。

列配置**按网关存在浏览器本地**（localStorage），换台机器要重新配。抽取路径是跟着
工具的参数结构走的，不同网关本来就不一样。

**其他**

- 点某一行**展开**看这次调用的入参和返回，两边并排；列表本身不带正文，展开时才按 `callId` 取。
  正文可以在「格式化」和「原文」之间切，也能整段复制
- 页头可选**自动刷新**（10 / 30 / 60 秒，默认关）。自动刷新带进来的新记录会闪一下
- 分页支持直接点页码；页数多到出现省略号时会额外给一个跳页输入框。每页 20 / 50 / 100
- 页头「**导出 Excel**」把**当前筛选结果**导成 `.xlsx`，列和表头取自当前的列配置。
  接口细节和行数上限见 [5.4 调用记录](#54-调用记录)
- 耗时列带一条按本页最大值归一的底纹，慢的那几条不用读数字就能挑出来
- 时间用本地时区显示，鼠标悬停看绝对时间
...
#### `GET /api/stats` — 统计（总览页）

一个时间窗内的聚合数字，**一次全部返回**：这一页上的所有图都是同一份数据的不同切法，
拆成多个请求只会带来"总量对得上、分组对不上"的中间态。

| 参数 | 说明 |
| --- | --- |
| `gatewayId` | 只看某个网关；省略表示全部网关。**只有指定了它，响应里的 `downstreams` 才有内容** —— 子 MCP 属于具体网关，跨网关放在一起没有意义 |
| `from` | 起始时间，ISO-8601 instant，含；省略表示 `to` 往前 24 小时 |
| `to` | 结束时间，ISO-8601 instant，不含；省略表示现在 |
...
# 最近 7 天，只看某个网关
curl -G 'http://127.0.0.1:8080/api/stats' \
  --data-urlencode 'gatewayId=<网关ID>' \
...
| 字段 | 内容 |
| --- | --- |
| `totals` | 窗口内的总量、各状态条数、成功率、平均和 P95 耗时 |
| `series` | 按时间桶展开的调用量（分状态）。**空桶也在里面**，调用方直接画就行 |
| `gateways` | 按网关分组。**调用量为 0 的网关也在列表里**，还带上网关自身的派生状态 |
| `downstreams` | 按子 MCP 分组，只在指定了 `gatewayId` 时非空 |
| `tools` | 调用量最高的 10 个聚合工具 |
| `errorCodes` | 出现最多的 8 个错误码 |

几条约定：

- **`bucket` 由窗口长度决定**：48 小时以内按 `HOUR`，更长按 `DAY`。桶按 UTC 切
  （库里存的就是 UTC），所以整点桶在任何整小时时区里都还是整点；半小时偏移的时区
  （如 `Asia/Kolkata`）看到的桶边界会落在半点上。
- **窗口最长 92 天**，超了报 `INVALID_REQUEST`。这不是性能限制 ——
  跨度再大用日桶画出来只是一条毛刺线，那时候该去查库做报表了。
- **比率和分位没有样本时是 `null`，不是 0**。"成功率 0%"是调用全失败了，
  "这段时间没有调用"是另一回事，界面上要画成不同的东西。
- **P95 只统计已结束的调用**。把还没跑完的 `STARTED` 当成 0 毫秒会把分位拉垮。
- **这个接口不返回任何调用正文**，底下的查询连 `request_json` / `response_json`
  都不 SELECT，所以它不在 [SECURITY.md](SECURITY.md) 那条"正文出口"的讨论范围内。

管理界面顶部的「总览」就是这个接口：四个概览数字 + 调用量趋势 + 网关/子 MCP/工具/错误码
四张分布图，点网关的柱子可以钻进那个网关。
