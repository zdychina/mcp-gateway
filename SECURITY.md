...
- **列表接口不带正文**。`GET .../call-records` 只返回摘要（工具名、状态、耗时、错误码），
  ...
  **唯一的开口是 `extract` 参数**（界面上的"正文列"）：调用方点名要哪几个字段，服务端按
  JSON Pointer 抽值，**只回抽到的值**——每个最长 200 字符、一次最多 4 个，正文本身在任何
  情况下都不进列表响应，由 `extractionStillNeverCarriesWholePayloads` 把关。
  一次列表请求能带走的业务内容因此从"几十条各接近 1 MiB"收敛到"几十条 × 800 字符"，
  差三个数量级 —— 但它不再是零。这是拿一点暴露面换排障效率，
  换不换由部署方决定：不配置正文列就不会触发，默认也不配。
  另外超过 64 KB 的正文根本不参与解析（`CallPayloadExtractor.MAX_PAYLOAD_CHARS`），
  既是内存保护，也让"能被成批抽走的东西"有个明确的天花板。
- **导出接口是这里最大的一个口子**。`GET .../call-records/export` 把筛选结果写成 xlsx，
  列由调用方指定，可以包含抽取列。一次最多 5000 行，超出即截断并在
  `X-Export-Truncated` 里说明；一行最多 4 个抽取值、每个 200 字符，
  所以单次导出能带走的正文片段上界约 4 MB（记录本身的工具名、状态、耗时等摘要字段不在此列）。
  它仍然不返回完整正文 —— 那只能按 `callId` 一条一条取。
  换句话说：它把"成批捞走业务片段"的门从抽取列的一页 20~100 条放宽到了 5000 条。
  这是为了让运维能把一段时间的调用捞进 Excel 里分析，代价明确写在这里；
  接受不了就把这个接口从反向代理上挡掉，界面上的按钮会因此报错，但列表和详情不受影响。
